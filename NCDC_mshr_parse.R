###########################################################################
# NCDC_mshr_parse.R; re-run 6/17/2017; rerun 5/3/2018
# Based off of redownload_stations.R
...
download.file("http://www.ncdc.noaa.gov/homr/file/mshr_enhanced.txt.zip", 
              destfile="mshr_enhanced.txt.zip")
############################################################################ 
##  manually unzip this and delete extraneous files that are not the .txt ##
############################################################################

##### Find mshr file and parse it!
print(list.files(pattern="mshr"))  
stations<-readLines(list.files(pattern="mshr"))
...
##################################################################
## open ncdcstation.csv in excel, and save as ncdcstation2.csv:  #
## it will drop quotes                                           #
##################################################################
stns<-read.csv("ncdcstation2.csv", strip.white=T, stringsAsFactors=F) # remove whitespace 
write.csv(stns, "ncdcstation2.csv", row.names=F)
