...
Tmax.tr60.leg<-as.expression(bquote(+.(round(Tmax.tr60, 2))~degree*C~dec^-1))
Tmin.tr60.leg<-as.expression(bquote(+.(round(Tmin.tr60, 2))~degree*C~dec^-1))
Tavg.tr60.leg<-as.expression(bquote(+.(round(Tavg.tr60, 2))~degree*C~dec^-1))
DTR.tr60.leg<-as.expression(bquote(.(round(DTR.tr60, 2))~degree*C~dec^-1))
...
legend("topleft", bty="n", 
       legend=c(Tmin.tr100.leg, Tmin.tr60.leg), 
       lty=c("33","12"), lwd=2,
...
legend("topleft", bty="n", 
       legend=c(Tavg.tr100.leg, Tavg.tr60.leg), 
       lty=c("33","12"), lwd=2,
...
legend("bottomleft", bty="n", 
       legend=c(DTR.tr100.leg, DTR.tr60.leg), 
       lty=c("33","12"), lwd=2,
...
plot(MinMaxTrend58$Tmax0.tr, pch=2, col="red", cex=1.5,
     ylim=c(-0.1, 0.35),
     xlab="Month", xaxt="n", yaxt = "n", cex.lab=1.5, cex.axis=1.3)
axis(side=4, labels=T, tick=T, cex.axis=1.3)
mtext(side=4, text=expression(Trend[1958-2017]~(degree*C~dec^-1)), 
       cex=1, line=4)
abline(0,0)
