...
RH_Hilo<-read.csv("NOT_USED/AKV_Sonde/UWyo/RH_layers_Hilo.csv", stringsAsFactors = F)
RH_Lih<-read.csv("NOT_USED/AKV_Sonde/UWyo/RH_layers_Lihue.csv", stringsAsFactors = F)

T_Hilo<-read.csv("NOT_USED/AKV_Sonde/UWyo/TempC_layers_Hilo.csv", stringsAsFactors = F)
T_Lih<-read.csv("NOT_USED/AKV_Sonde/UWyo/TempC_layers_Lihue.csv", stringsAsFactors = F)

thte_Hilo<-read.csv("NOT_USED/AKV_Sonde/UWyo/ThetaeK_layers_Hilo.csv", stringsAsFactors = F)
thte_Lih<-read.csv("NOT_USED/AKV_Sonde/UWyo/ThetaeK_layers_Lihue.csv", stringsAsFactors = F)
...
##### Compare Tmax
plot(Tdat.80$coefs_Tmax.b~Tdat.80$Year,
     type="l", col="black", lwd=2)
plot(Tdat.80$coefs_Tmax.m~Tdat.80$Year,
     type="l", col="black", lwd=2)

plot(cphto.80$phto_OBday.z~cphto.80$Year, type="l", col="red", 
     ylim=range(cphto.80$phto_OBday.z, cphto.80$phto_OBSFday.z, na.rm=T))
lines(cphto.80$phto_OBSFday.z~cphto.80$Year, col="purple")

plot(cphto.80$phto_OBday.fr~cphto.80$Year, type="l", col="red",
     ylim=range(cphto.80$phto_OBday.fr, cphto.80$phto_OBSFday.fr, na.rm=T))
lines(cphto.80$phto_OBSFday.fr~cphto.80$Year, col="purple")

plot(sHilo.80$Mean.mixed.layer.mixing.ratio~sHilo.80$Year, 
     type="l", col="red")
plot(sHilo.80$Precipitable.water..mm..for.entire.sounding~sHilo.80$Year, 
     type="l", col="red")
plot(sHilo.80$Pres..hPa..of.the.Lifted.Condensation.Level~sHilo.80$Year, 
     type="l", col="red")
plot(sHilo.80$LFCT.using.virtual.temperature~sHilo.80$Year, 
     type="l", col="red")

par(mfrow=c(5,5))
for (i in 2:25){   # look at all the indices over 1980-2017
...
par(mfcol=c(2,2))
plot(cphto.80$phto_OBday.z~cphto.80$Year, col="red",type="l",
     ylim=range(cphto.80$phto_OBday.z, cphto.80$phto_OBSFday.z, na.rm=T))
lines(cphto.80$phto_OBSFday.z~cphto.80$Year, col="purple")
plot(sHilo.80$Pres..hPa..of.the.Lifted.Condensation.Level~sHilo.80$Year, col="red", type="l")

summary(lm(cphto.80$phto_OBSFday.z~sHilo.80$Pres..hPa..of.the.Lifted.Condensation.Level))
plot(cphto.80$phto_OBSFday.z~sHilo.80$Pres..hPa..of.the.Lifted.Condensation.Level)

plot(cphli.80$phli_OBday.z~cphli.80$Year, col="red",type="l",
     ylim=range(cphli.80$phli_OBday.z, cphli.80$phli_OBSFday.z, na.rm=T))
lines(cphli.80$phli_OBSFday.z~cphli.80$Year, col="purple")
plot(sLih.80$Pres..hPa..of.the.Lifted.Condensation.Level~sLih.80$Year, col="red", type="l")

summary(lm(cphli.80$phli_OBSFday.z~sLih.80$Pres..hPa..of.the.Lifted.Condensation.Level))
plot(cphli.80$phli_OBSFday.z~sLih.80$Pres..hPa..of.the.Lifted.Condensation.Level)


## Compare daytime frequency of OB/OBSF to LCL

#pdf("MDDY_Sfc_Sound_Ceil.pdf", width=8, height=9)
par(mfcol=c(3,2), mar=c(2,4,3,4))

# Surface
plot((Tdat.80$coefs_Tmax.m)*(-1) ~Tdat.80$Year, 
     col="black",type="l", main="Surface",
     ylab="-dT/dz", xlab="Year")
par(new=T)
plot(Tdat.80$coefs_Tmax.b ~ Tdat.80$Year, col="red", type="l", lty=2,
     yaxt="n", ylab="", xlab="")
axis(4)
mtext("Tmax (C)", side=4, line=3,cex=0.7)

# Sounding
plot(RH_Hilo.80$RH_800.1600m ~RH_Hilo.80$Year, 
     col="dark gray",type="l",lty=2, main="Hilo soundings",
     ylab="RH", xlab="Year", ylim=c(60,85))
lines(RH_Hilo.80$RH_0.1600 ~RH_Hilo.80$Year, col="blue")
lines(RH_Hilo.80$RH_0.50m ~RH_Hilo.80$Year, col="red")
par(new=T)
plot(sHilo.80$Pres..hPa..of.the.Lifted.Condensation.Level~sHilo.80$Year, 
     col="black", type="l", ylim=c(890,930), yaxt="n", ylab="", xlab="")
axis(4)
mtext("LCL pressure (hPa)", side=4, line=3,cex=0.7)

# Ceilometer
plot(cphto.80$phto_OBday.fr~cphto.80$Year, col="red",type="l",
     ylab="Frequency", xlab="Year", ylim=c(0,0.2), main="Hilo ceilometer")
lines(cphto.80$phto_OBSFday.fr~cphto.80$Year, col="purple")
lines(cphto.80$phto_CLRday.fr~cphto.80$Year, col="black")

par(new=T)
plot(cphto.80$phto_OBday.z~cphto.80$Year, 
     col="red",type="l",lty=2, yaxt="n",
     ylab="", xlab="", ylim=c(600,1800))
lines(cphto.80$phto_OBSFday.z~cphto.80$Year, col="purple", lty=2)
axis(4)
...
par(new=T)
plot(sLih.80$Pres..hPa..of.the.Lifted.Condensation.Level~sLih.80$Year, 
     col="black", type="l", ylim=c(890,930), yaxt="n", ylab="", xlab="")
axis(4)
mtext("LCL pressure (hPa)", side=4, line=3,cex=0.7)

# Ceilometer
plot(cphli.80$phli_OBday.fr~cphli.80$Year, col="red",type="l",
     ylab="Frequency", xlab="Year", ylim=c(0,0.2), main="Lihue ceilometer")
lines(cphli.80$phli_OBSFday.fr~cphli.80$Year, col="purple")
lines(cphli.80$phli_CLRday.fr~cphli.80$Year, col="black")

par(new=T)
plot(cphli.80$phli_OBday.z~cphli.80$Year, col="red",type="l",lty=2, yaxt="n",
     ylab="", xlab="", ylim=c(600,1800))
lines(cphli.80$phli_OBSFday.z~cphli.80$Year, col="purple", lty=2)
axis(4)
mtext("Ceiling height (m)", side=4, line=3,cex=0.7)
